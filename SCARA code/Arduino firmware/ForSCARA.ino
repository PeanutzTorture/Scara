...
int stepsPermm = 240; //96.96; // Steps per mm travel for Z-axis
int joint1Home = -125; // Angle from straight in degrees, CCW positive
int joint2Home = -137;
int joint3Home = 142;
int zHome = -5; // distance in mm down from top of Z-travel
...
void setup() {
  ...
  stepper1.setMaxSpeed(3000); //// From Accelstepper: "Max possible speed depends on clock speed. Caution: Speeds that exceed the maximum speed supported by the processor may Result in non-linear accelerations and decelerations"
...
void loop() {
    ...
    // Ensure all steppers run simultaneously
    stepper1.run();
    stepper2.run();
    stepper3.run();
    stepper4.run();
